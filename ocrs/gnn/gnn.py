...
class GRUGNNCell(nn.Module):
    def __init__(self, slot_size, gnn_hidden_dim, num_objects, act=torch.tanh, update_bias=-1):
        ...
        self._gnn = GNN(2 * self._slot_size, self._gnn_hidden_dim, -1, self._num_objects, ignore_action=True,
                        copy_action=False, layer_norm=True, num_layers=3, use_interactions=True, edge_actions=False,
                        output_dim=3 * self._slot_size)

    def forward(self, updates, slots_prev):
        node = torch.cat([updates, slots_prev], dim=-1)
        parts = self._gnn(node, action=None)[0]
        reset, cand, update = torch.split(parts, [self._slot_size] * 3, dim=-1)
