...
def get_dataloaders(config, batch_size, num_workers, replace=False, shuffle_on_validation=False):
    ...
    if config.episode_dataset:
        ...
        train_dl = DataLoader(
            EpisodesDataset(datafile, mode='train', obs_size=config.obs_size, extension=config.extension,
        ...
        val_dl = DataLoader(
            EpisodesDataset(datafile, mode='val', obs_size=config.obs_size, extension=config.extension,
                            augmentation_probability=config.get('augmentation_probability', 0),
                            episode_folder_pattern=config.get('episode_folder_pattern', '*'),
